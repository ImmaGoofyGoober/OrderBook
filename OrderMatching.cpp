...
void OrderMatching::orderMatching() {
    while (!orderbook.bidOrders.empty() && !orderbook.askOrders.empty()) {
        ...
        uint64_t bidPrice = bidIterator->first.first;
        uint64_t askPrice = askIterator->first.first;
        ...
        if (bidPrice == 0 || askPrice == 0 || bidPrice < askPrice || bidQuantity == 0 || askQuantity == 0) {
